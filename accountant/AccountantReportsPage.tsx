...
const getDynamicTransactionPaidLabel = (key: string) => {
  ...
  const formatted = formatHeaderLabel(withoutSuffix);

  // Removes " Paid" from the end of the final header string
  return formatted.replace(/\s+Paid$/i, "");
};
...
const AccountantReportsPage: React.FC = () => {
  ...
  const fetchMetadata = useCallback(async () => {
    ...
    try {
      const [classRes, sectionRes] = await Promise.all([
        axios.get(`https://cleezoclass.com:5000/api/admin/classes?schoolCode=${schoolCode}`),
        axios.get(`https://cleezoclass.com:5000/api/admin/sectionFilter?schoolCode=${schoolCode}`),
      ]);
  ...
  const fetchSummaryDue = useCallback(async () => {
    ...
    try {
      ...
      if (selectedYear === "2024-2025") {
        ...
        const response = await axios.get(`https://cleezoclass.com:5000/api/fee-records-alldata?${params.toString()}`);
      ...
      const response = await axios.get(`https://cleezoclass.com:5000/api/fees-summary-ledgerData?${params.toString()}`);
  ...
  const handleTodayLedger = async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://cleezoclass.com:5000/api/ledger?${params.toString()}`);
  ...
  const handleDiscountsReport = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(`https://cleezoclass.com:5000/api/discounts-report?${params.toString()}`);
      const result = Array.isArray(response.data) ? response.data : response.data?.data || [];
...
const dynamicTransactionPaidKeys = useMemo(
  () =>
    activeView === "studentTransactions"
      ? getDynamicTransactionPaidKeys(activeRows).filter((key) => {
          const lowerKey = key.toLowerCase();
          // Exclude installments, generic "amount", and "amount_paid"
          return (
            !lowerKey.includes("installment") && 
            lowerKey !== "amount" && 
            lowerKey !== "amount_paid"
          );
        })
      : [],
  ...
  const dynamicCompleteFeeBases = useMemo(() => {
    ...
    const candidateBases = completeFeeTypeBases.length ? completeFeeTypeBases : getDynamicFeeBases(activeRows);
    // Only keep fee types that actually have an amount or a paid value for at
    // least one student - fee types that are entirely 0 across the report
    // are dropped instead of rendering an all-zero pair of columns.
    return candidateBases.filter((baseKey) =>
      activeRows.some(
        (row) =>
          toAmount(getDynamicFeeValue(row, baseKey)) > 0 ||
          toAmount(getDynamicPaidValue(row, baseKey)) > 0
      )
    );
  ...
  const visibleCompleteStaticColumns = useMemo(() => {
    if (activeView !== "complete") return [];
    return [
      { key: "CompleteFee", label: "Total Fee", getValue: (row: ReportRow) => toAmount(row.CompleteFee) },
  ...
  const visibleTransactionStaticColumns = useMemo(() => {
    ...
    const columns = [
      { key: "Admission_paid", label: "Admission", getValue: (row: ReportRow) => toAmount(row.Admission_paid) },
      { key: "books_paid", label: "Books ", getValue: (row: ReportRow) => toAmount(row.books_paid) },
      { key: "uniform_paid", label: "Uniform", getValue: (row: ReportRow) => toAmount(row.uniform_paid) },
      { key: "exam_paid", label: "Exam ", getValue: (row: ReportRow) => toAmount(row.exam_paid) },
      { key: "bus_paid", label: "Bus", getValue: (row: ReportRow) => toAmount(row.bus_paid) },
  ...
  const activeReportTitle = useMemo(() => {
    switch (activeView) {
      ...
      case "paid":
        return "Paid";
      case "discounts":
        return "Discounts Report";
      case "referrals":
        return "Referrals";
      case "feeType":
        return "Fee Type Report";
      case "collectionSummary":
    ...
    (view: ReportView, rows: ReportRow[]) => {
      ...
      if (view === "ledger") {
        return [
          ...
          { key: "amount_paid", label: "Total", getValue: (row: ReportRow) => formatMoney(row.amount_paid) },
        ];
      }
 ...
 if (view === "complete") {
  ...
  return cols;
    ...
    .forEach((key) => {
      cols.push({
        key,
        label: getDynamicTransactionPaidLabel(key),
  ...
  dynamicDiscountColumns.forEach((key) => {
    ...
    cols.push({
      key,
label: formatHeaderLabel(key).replace(/\s*Discount\s*$/i, ""),      getValue: (row: ReportRow) => formatMoney(row[key] || 0),
    });
  });
...
if (view === "referrals") {
  ...
  dynamicDiscountColumns.forEach((key) => {
    ...
    cols.push({
      key,
label: formatHeaderLabel(key).replace(/\s*Discount\s*$/i, ""),      getValue: (row: ReportRow) => formatMoney(row[key]),
    });
  });
...
if (view === "paid" || view === "main" || view === "unpaid") {
  ...
  const filteredHeaders = rawHeaders.filter((header) => {
    ...
    // Exclude unwanted system or hidden keys from reports
    return !['id', 'login_id', 'created_at', 'updated_at', 'academic_year', 'remarks'].includes(lower);
  });
  ...
  return filteredHeaders.map((header) => ({
    ...
    label:
      view === "main"
        ? formatDueReportHeaderLabel(header)
        : header === "Class_name"
        ? "Class"
        : header === "StudentName"
        ? "Student Name"
        : header === "Total_Expected"
        ? "Total Fee"
...
const handleDownloadPDF = useCallback(() => {
  ...
  doc.text(
    `Rows: ${activeRows.length} | Generated On: ${dateStr} ${timeStr}`,
    14,
    22
  );
  ...
  const filename = `accountant-${activeReportTitle
    .toLowerCase()
    .replace(/\s+/g, "-")}-${fileDate}-${fileTime}.pdf`;
...
const handleDownloadReport = useCallback(() => {
  ...
  const worksheet = XLSX.utils.aoa_to_sheet([
    [
      activeReportTitle,
      ...Array(Math.max(columns.length - 1, 0)).fill(""),
    ],
    [
  ...
  XLSX.writeFile(
    workbook,
    `accountant-${activeReportTitle
      .toLowerCase()
      .replace(/\s+/g, "-")}-${fileDate}-${fileTime}.xlsx`
  );
}, [activeReportTitle, activeRows, activeView, getTableColumns]);
const handlePrintReport = useCallback(() => {
  // Get filtered column list
  let columns = getTableColumns(activeView, activeRows);
  if (!columns.length) return;

  // 1. Prepare rows specifically for Discounts & Referrals print
  let printableRows = activeRows;

  if (activeView === "discounts" || activeView === "referrals") {
    const splitRows = [];

    activeRows.forEach((row) => {
      // Find all dynamic fee type columns available in the table
      const feeColumns = dynamicDiscountColumns.filter((col) => col !== "Discount");

      feeColumns.forEach((targetCol) => {
        const amount = Number(row[targetCol] || 0);

        // Create a dedicated row whenever a fee type has a discount > 0
        if (amount > 0) {
          const singleFeeRow = { ...row };

          // Reset all fee column values to 0.00 for this row
          feeColumns.forEach((col) => {
            singleFeeRow[col] = 0;
          });

          // Set only the current target fee type amount and update Total Discount
          singleFeeRow[targetCol] = amount;
          singleFeeRow["total_discount"] = amount;
          singleFeeRow["Total Discount"] = amount;

          splitRows.push(singleFeeRow);
        }
      });
    });

    // If split rows were generated, use them for printing
    if (splitRows.length > 0) {
      printableRows = splitRows;
    }
  }
    ...
    columns = columns.filter((col) => {
      ...
      return (
        ...
        !NON_FEE_REPORT_KEYS.has(lowerKey) &&
        !lowerLabel.includes("paid")
      );
    });

    const totalPaidIndex = columns.findIndex((col) => col.label === "Total Paid");
    const summaryColumns = [
      ...
      {
        key: "Total_Discounts_Print",
        label: "Total Discounts",
        getValue: (row) => formatMoney(getRowDiscountTotal(row)),
      },
    ];

    if (totalPaidIndex !== -1) {
      columns.splice(totalPaidIndex, 0, ...summaryColumns);
    }
  }

  // 2. Build table HTML from printableRows
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .print-meta { font-size: 14px; margin: 0 0 10px; color: #4b5563; }
          table { width: 100%; border-collapse: collapse; margin-top: 10px; }
          th, td { border: 1px solid #cfd6dd; padding: 8px; font-size: 11px; text-align: left; vertical-align: top; }
      ...
      <body>
        ...
        <div class="print-meta">Records: ${printableRows.length} | Printed on: <b>${escapeHtml(currentFormattedDateTime)}</b></div>
        <table>
          <thead>
            <tr>
              <th>S.NO.</th>
  ...
  printWindow.focus();
  printWindow.print();
}, [activeReportTitle, activeRows, activeView, dynamicDiscountColumns, getTableColumns]);
...
if (activeView === "main") {
const rawHeaders = activeRows.length
  ? Object.keys(activeRows[0]).filter((header) => {
      const label = formatDueReportHeaderLabel(header).toLowerCase();
      // Discounts are never shown on the Dues report, only the amount + due
      return !label.includes("paid") && !label.includes("discount");
    })
  : [];

// For each fee type, show only the Due column - the base fee amount column is
// dropped. A fee type is hidden entirely when it has no amount for any student
// (e.g. a fee type like "Ran" or "Abacus" that isn't charged to this batch).
const dueHeaders = rawHeaders.filter((header) => header.toLowerCase().endsWith("_due"));
const matchedBaseHeaders = new Set<string>();
const emptyFeeTypeHeaders = new Set<string>();

dueHeaders.forEach((dueHeader) => {
  const baseHeader = rawHeaders.find(
    (header) => header.toLowerCase() === dueHeader.slice(0, -"_due".length).toLowerCase()
  );
  if (!baseHeader) return;

  matchedBaseHeaders.add(baseHeader);

  const hasAmount = activeRows.some((row) => toAmount(row[baseHeader]) > 0);
  if (!hasAmount) {
    emptyFeeTypeHeaders.add(baseHeader);
    emptyFeeTypeHeaders.add(dueHeader);
  }
});

const headers = rawHeaders.filter(
  (header) => !emptyFeeTypeHeaders.has(header) && !matchedBaseHeaders.has(header)
);
  return (
    <table className="accountant-reports-table">
      <thead>
...
if (activeView === "complete") {
 ...
 if (activeView === "discounts" || activeView === "referrals") {
  ...
  // Convert each fee type discount into a separate row
  const discountRows = [];
  ...
  paginatedRows.forEach((row) => {
    dynamicDiscountColumns
      ...
      .forEach((column) => {
        ...
        // Only show rows having discount > 0
        if (amount > 0) {
          discountRows.push({
            ...row,
            feeType: formatHeaderLabel(column).replace(/\s*Discount$/i, ""),
  ...
  return (
    <table className="accountant-reports-table"><thead>
  <tr>
    <th>S.NO.</th>
    <th>Student Name</th>
    <th>Class</th>
    ...
    {dynamicDiscountColumns
      ...
      .map((column) => (
        <th key={column}>
          {formatHeaderLabel(column).replace(/\s*Discount$/i, "")}
        </th>
      ))}
...
<div className="accountant-reports-toolbar-actions">
  <select 
    ...
    style={{ padding: "6px 10px", borderRadius: "4px", border: "1px solid #cfd6dd", background: "#fff",marginTop:"15px" }}
  >
