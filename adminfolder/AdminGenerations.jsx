...
const API_BASE = "https://cleezoclass.com:5000/api";
...
const ID_CARD_PDF_WIDTH_MM = 100;
const ID_CARD_PDF_HEIGHT_MM = 100;
...
const resolveIdCardPhotoUrl = (rawPhoto) => {
  ...
  if (trimmed.startsWith("/public/uploads/")) {
    return `https://cleezoclass.com:5000${trimmed.replace("/public", "")}`;
  }
  if (trimmed.startsWith("public/uploads/")) {
    return `https://cleezoclass.com:5000/${trimmed.replace(/^public\//, "")}`;
  }
...
//   if (!trimmed.startsWith("/")) return `https://cleezoclass.com:5000/${trimmed}`;
//   return `https://cleezoclass.com:5000${trimmed}`;
// };
      ...
      const getOverallPerformance = async () => {
          ...
          try {
              ...
              const response = await axios.get(
                  "https://cleezoclass.com:5000/api/overall-performance-percentage",
                  {
                      params: {
                          schoolCode,
                      },
...
const handlePrintSelectedClassIdCards = async () => {
  ...
  const fetchStudentsForReportRange = async () => {
    ...
    const attempts = [
      ...
      () => axios.get(`https://cleezoclass.com:5000/students-details`, { params: { schoolCode: code } }),
      () => axios.get(`https://cleezoclass.com:5000/api/students`, { params: { schoolCode: code } }),
      () => axios.post(`https://cleezoclass.com:5000/api/students`, { schoolCode: code }),
    ];
  ...
  const enrichStudentsForReports = async (students) => {
    ...
    const results = await Promise.all(
      Array.from(grouped.values()).map(async ({ className, section, students: groupedStudents }) => {
        try {
          const response = await axios.get(
            `https://cleezoclass.com:5000/api/studentsName/${encodeURIComponent(className)}?schoolCode=${encodeURIComponent(
              schoolCode
...
useEffect(() => {
  ...
  axios
    .post("https://cleezoclass.com:5000/api/users", {
      schoolCode,
      user_type: "teacher",
    })
    .then((res) => {
...
const handleOpenTeacherIdCardGeneration = async (teacherIdOverride = null, templateOverride = null) => {
  ...
  const fetchStudentReportPayload = async (student, currentSchoolCode) => {
    ...
    if (className && section && (!student.father_name && !student.fatherName)) {
      try {
        const studentsRes = await axios.get(
          `https://cleezoclass.com:5000/api/studentsName/${encodeURIComponent(className)}?schoolCode=${encodeURIComponent(currentSchoolCode)}&section=${encodeURIComponent(section)}`
        );
    ...
    const performanceAttempts = [
      ...
      () => axios.post(`https://cleezoclass.com:5000/api/overall/academic-performance`, performancePayload),
    ];
    ...
    const attendanceAttempts = [
      ...
      () => axios.post(`https://cleezoclass.com:5000/api/report/attendance/monthly`, performancePayload),
    ];
...
//     // Only request CORS-mode loading for data: URIs (always same-origin-safe,
//     // never blocked). For http(s) photo URLs, DO NOT force crossOrigin —
//     // the photo host (cleezoclass.com:4000) doesn't send CORS headers, so
//     // crossOrigin="anonymous" makes the browser refuse to load the image
//     // at all (blank/broken photo), even though it loads fine as a normal
//     // <img> without that attribute. This mirrors the fix already applied
//     // below for the school background image.
...
//   if (photo) {
//     doc.querySelectorAll("img[data-photo], img[data-student-photo]").forEach((img) => {
//       // See note above: forcing crossOrigin="anonymous" on a non-CORS-enabled
//       // external photo host makes the image fail to load entirely.
...
const syncIdCardPhotoBeforeCapture = async (doc, student = null, photoCache = null) => {
  ...
  // Apply to all student photo elements
  // if (studentPhotoData) {
  //   doc.querySelectorAll("img[data-photo], img[data-student-photo], img.student-photo, .photo img").forEach((img) => {
  //     if (studentPhotoData.startsWith("data:")) {
  //       img.crossOrigin = "anonymous";
  //     } else {
  //       img.removeAttribute("crossorigin");
  //     }
  ...
  //     finalSrc = studentPhotoData;
  //   } else {
  //     const resolved = readIdCardStoredMedia(doc, rawSrc);
  //     finalSrc = isPhotoLike ? resolveIdCardPhotoUrl(resolved) : resolved;
  //   }
  ...
  if (studentPhotoData) {
    ...
    doc.querySelectorAll(photoSelectors).forEach((img) => {
      ...
      // Also sync background-image on parent frame if the template uses has-bg-photo (e.g. idcard6.html)
      const photoFrame = img.closest(".photo, .photo-frame, .student-photo, .student-photo-box");
      if (photoFrame) {
        photoFrame.style.backgroundImage = `url("${studentPhotoData}")`;
  ...
  // const principalPhoto = readIdCardStoredMedia(doc, params.get("principalPhoto"));
  // const principalPhotoFallback = readIdCardStoredMedia(doc, params.get("principal"));
  ...
  doc.querySelectorAll("img[data-principal-photo]").forEach((img) => {
    const currentSrc = img.getAttribute("src") || img.src || "";
    // const resolvedSrc = normalizeForIframe(principalPhoto || principalPhotoFallback || currentSrc);
...
//     iframe.style.top = '0';
//     iframe.style.width = `${ID_CARD_CAPTURE_WIDTH}px`;
//     iframe.style.height = `${ID_CARD_CAPTURE_HEIGHT}px`;
...
const exportPreviewTilesAsPdf = async (canvases, config) => {
  const { paperSize, fileName, printOptions = {}, targetType } = config;
  const basePage = getPdfPageSize(paperSize);
  const pageWidth = basePage.width;
  const pageHeight = basePage.height;
  const margin = 8;
  const gap = 6;
  const cols = 2;
  const tileCanvasWidth = 760;
  const tileCanvasHeight = 650;
  const tileRatio = tileCanvasWidth / tileCanvasHeight;
  const tileWidth = (pageWidth - margin * 2 - gap) / cols;
  const tileHeight = tileWidth / tileRatio;
  const rows = Math.max(1, Math.floor((pageHeight - margin * 2 + gap) / (tileHeight + gap)));
  const perPage = cols * rows;

  // 🔥 Group by the render-time studentKey (index-based), never by name/id —
  // duplicate or missing names/ids would otherwise merge different students
  // into one group and overwrite their front/back canvases (front of one
  // student paired with back of another).
  const grouped = new Map();
  canvases.forEach(({ student, side, canvas, studentKey }, i) => {
    const groupId = studentKey || `student-${i}`;
    if (!grouped.has(groupId)) grouped.set(groupId, { student, frontCanvas: null, backCanvas: null });
    const group = grouped.get(groupId);
    if (side === "back") group.backCanvas = canvas;
    else group.frontCanvas = canvas;
  });

  const tiles = Array.from(grouped.values()).map((group) => createIdCardPreviewTileCanvas({ ...group, targetType }));
  const pdf = new jsPDF({
    orientation: "portrait",
    unit: "mm",
  ...
  tiles.forEach((tileCanvas, index) => {
    ...
    pdf.addImage(imgData, "JPEG", x, y, tileWidth, tileHeight);
  ...
  if (printOptions.pageNumbers) {
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      pdf.text(`Page ${p} of ${totalPages}`, pageWidth / 2, pageHeight - 3, { align: "center" });
    }
  }
...
const exportReportCardsAsSinglePdf = async (canvases, config) => {
  const { paperSize = 'a4', fileName = 'ReportCards', printOptions = {} } = config;
...
const exportReportCardsPdfAsZip = async (canvases, config) => {
  const { fileName = 'ReportCards_Export', paperSize = 'a4', printOptions = {} } = config;
  const zip = new JSZip();
  const folder = zip.folder(fileName);
  const basePage = getReportCardPageDimensions(paperSize);
...
const precomputeReportCardRanks = async () => {
  const currentSchoolCode = localStorage.getItem("schoolCode") || schoolCode;
  const students = reportRangeStudents;
  ...
  try {
    for (let i = 0; i < students.length; i++) {
      ...
      setDownloadProgress({
        ...
        current: i + 1,
        total: students.length,
        message: `Calculating ranks (${i + 1}/${students.length}): ${student?.name || student?.student_name || "Student"}`,
      });
        ...
        <div className="admin-events-download-section">
          ...
          <div className="admin-events-radio-group">
            <label>
              <input
                type="radio"
                name="reportCardRankMode"
                value="without-rank"
                checked={reportCardRankMode === 'without-rank'}
                onChange={e => {
                  setReportCardRankMode(e.target.value);
                  reportRankMapRef.current = new Map();
                }}
              /> Without Rank
            </label>
          ...
          {reportCardRankMode === 'with-rank' && (
            <small style={{ color: '#888', display: 'block', marginTop: '4px', fontSize: '12px' }}>
              {reportRankComputing
                ? 'Calculating ranks across all selected students…'
                : `Rank is based on total marks across all ${reportRangeStudents.length} student(s) in this class/section. Students tied on total marks receive the same rank.`}
            </small>
          )}
        </div>
