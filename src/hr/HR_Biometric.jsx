...
const Biometric = () => {
  ...
  //         `https://cleezoclass.com:5000/getUserName?username=${encodeURIComponent(
  //           username
  //         )}`
  //       );
...
const name = localStorage.getItem("name")
  const fetchDeletedUsers = async () => {
    try {
      const res = await axios.get(
        "https://cleezoclass.com:5000/api/deleted-students",
        { params: { schoolCode } }
      );
  ...
  const fetchLeaveRequests = async () => {
    try {
      const res = await axios.get(
        "https://cleezoclass.com:5000/leave-requests-list",
        { params: { schoolCode } }
      );
