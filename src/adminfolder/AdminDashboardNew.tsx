...
const ADMIN_API_BASE = "https://cleezoclass.com:5000/api";
  ...
  const getBehaviourPercentage = async () => {
    try {
      ...
      const res = await axios.get(
        "https://cleezoclass.com:5000/api/overall-behaviour-percentage",
        {
          params: { schoolCode },
        }
      );
...
console.log("attenance percentage",attendanceWithPercentage);
 ...
 console.log(attendanceRows.length)
  ...
  useEffect(() => {
    ...
    axios
      .get("https://cleezoclass.com:5000/api/school-festival", {
        params: { schoolCode },
      })
      .then((res) => {
  ...
  const normalizeUserPhoto = (rawPhoto: any) => {
    ...
    if (photo.startsWith("/uploads/")) {
      return `https://cleezoclass.com:5000${photo}`;
    }
  ...
  const loadPendingChats = async () => {
    ...
    const schoolCode = localStorage.getItem("schoolCode") || "NOVA";
    const endpoints = [
      `https://cleezoclass.com:5000/api/chat-requests/pending?schoolCode=${encodeURIComponent(schoolCode)}`,
      `https://cleezoclass.com:5000/api/chat-requests?schoolCode=${encodeURIComponent(schoolCode)}`,
    ];
    try {
      ...
      const billDeleteRes = await fetch(
        `https://cleezoclass.com:5000/api/bill/delete-requests?schoolCode=${encodeURIComponent(schoolCode)}&status=pending`
      );
      if (billDeleteRes.ok) {
    ...
    const fetchStoreActions = async () => {
      ...
      try {
        const res = await fetch(
          `https://cleezoclass.com:5000/api/po/requests?schoolCode=${encodeURIComponent(schoolCode)}`
        );
    ...
    <div className="admin-dashboard-page">
      <div className="admin-dashboard-shell">
        {String(localStorage.getItem("userRole") || "").toLowerCase() === "superadmin" ? (
          <button
            className="admindashboard-nav-chiefdashboard"
            type="button"
        ...
        <aside className="admin-sidebar-strip">
          {sidebarItems.map((item) => (
            <button
              key={item.key}
              type="button"
              className={`admin-sidebar-item ${activeSidebar === item.key ? "admin-sidebar-item-active" : ""}`}
              onClick={() => {
        ...
        <div className="admin-main-area">
          <div className="admin-topbar">
            <div className="admin-topbar-left">
              {topTabs.map((tab) => (
                <div
                  key={tab}
                  className={`admin-topbar-tab ${activeTopTab === tab ? "admin-topbar-tab-active" : ""}`}
                >
                  <button
                    className="admin-topbar-tab-button"
                    type="button"
                    onClick={() => {
                      setActiveTopTab(tab);
                      if (tab === "Academics") {
            ...
            <div className="admin-topbar-right">
              <div className="header-profile-wrap" ref={userDropdownRef}>
                <div className="admin-header-actions">
                  <button
                    className="admin-help-icon-btn"
                    onClick={() => setIsHelpOpen(true)}
                    title="Help"
                  >
                    <FiHelpCircle className="admin-help-icon" />
                  </button>
          ...
          <div className="admin-grid">
            <div className="admin-row admin-row-top">
              ...
              <div className="admin-task-card admin-card">
              ...
              <div className="admin-mini-cards">
                {quickCards.map((card) => (
                  <div
                    key={card.title}
                    className="admin-quick-card admin-card"
            ...
            <div className="admin-row admin-row-middle">
              <div className="admin-academics-card admin-card">
                <div className="admin-card-header">
                  <h3>Academics</h3>
                  <button type="button" className="admin-card-filter">
                ...
                <button
                  type="button"
                  className="admin-inline-plus admin-inline-plus-below"
                ...
                <div className="admin-academics-content">
                  <div
                    className="admin-progress-panel"
                    style={{ ["--admin-progress" as any]: progressAngle }}
                  >
                  ...
                  <div className="admin-academics-stats">
                    <p>
                      <span>Performance - student:</span>{" "}
                      <strong>{performance.overallPercentage || 0}%</strong>
                    </p>
                    <p>
                      <span>Positive:</span>{" "}
                      <strong>{behaviour.positivePercentage || 0}%</strong>
                    </p>
                    <p>
                      <span>Needs Improvement:</span>{" "}
                      <strong>{behaviour.needsImprovementPercentage || 0}%</strong>
                    </p>
                    <p>
                      <span>Negative:</span>{" "}
                      <strong>{behaviour.negativePercentage || 0}%</strong>
                    </p>
                  </div>
                </div>
              </div>

              <div className="admin-events-card admin-card admin-events-calendar">
              ...
              <div className="admin-livechat-card admin-card">
                <div className="admin-card-header">
                  <h3>
                  ...
                  <div className="admin-card-filters">
                    <button className="admin-card-filter" type="button" onClick={openLiveChatPopup}>
                      + Create New
                    </button>
                    <div className="admin-livechat-count">
                      <strong>
                        {activeQuickPanel === "assistant"
                ...
                {activeQuickPanel === "assistant" ? (
                  <div className="admin-assistant-list">
                    {festival && (
                      <div
                        className="admin-events-assistant-item admin-events-assistant-posters"
                    ...
                    {eventSelected ? (
                      ...
                      assistantPanelItems.map((item) => (
                        <div key={item.title} className="admin-assistant-card">
                          <div className="admin-assistant-card-title">{item.title}</div>
                          <div className="admin-assistant-card-desc">{item.desc}</div>
                        </div>
                      ))
                    )}
                  </div>
                ) : activeQuickPanel === "storepo" ? (
                  <div className="admin-storepo-list">
                    {storeActionsLoading && (
                    ...
                    {storeActions.map((action: any) => (
                      <div
                        ...
                        className="admin-storepo-card"
                      >
                        <div className="admin-storepo-card-title">
                          {action.text ||
                            `${action.stockName || "Stock"} • Qty ${action.quantity || "-"} • ${action.category || "PO"}`}
                        </div>
                        <div className="admin-storepo-card-desc">
                  ...
                  <div className="admin-livechat-list">
                    {pendingChatsLoading && (
                    ...
                    {!pendingChatsLoading && !pendingChatsError && pendingChats.length > 0 && (
                      <>
                        <div className="admin-livechat-group">
                          <small className="admin-livechat-group-title">Requests</small>
                              ...
                              >
                                {chat.requestType === "bill-delete" ? (
                                  <>
                                    ...
                                    <div className="admin-livechat-actions">
                                      <button
                                        type="button"
                                        onClick={() => handleBillDeleteApproval(chat, "approve")}
                                        className="admin-livechat-btn-approve"
                                      >
                                        Approve
                                      </button>
                                      <button
                                        ...
                                        className="admin-livechat-btn-reject"
                                      >
                                        Reject
                                      </button>
                                    </div>
                        ...
                        <div className="admin-livechat-group">
                          <small className="admin-livechat-group-title">Scheduled</small>
                          {scheduledItems.length === 0 ? (
                            ...
                            scheduledItems.map((chat: any) => (
                              ...
                              >
                                ...
                                <div className="admin-livechat-actions">
                                  <button className="admin-livechat-icon-btn">▷</button>
                                  <button className="admin-livechat-icon-btn">✕</button>
                                </div>
                              </div>
                            ))
            ...
            <div className="admin-row admin-row-bottom">
              <div className="admin-ticket-card admin-card">
                <FrontDesk_Tickets showTable={false} />
              </div>

              <div
                className="admin-premium-card admin-card"
                role="button"
                tabIndex={0}
              ...
              >
                <img src={premiumIcon} alt="Premium" className="admin-premium-icon" />
                <h3>Go Premium!</h3>
              ...
              <div className="admin-bottom-right">
                <div className="admin-performance-card admin-card">
                  <div
                ...
                <div className="admin-substitute-card admin-card">
                  <p className="admin-substitute-amount">8 abs / {teachers.length} avl</p>
                  <h3>Substitute</h3>
                  <p>8 teachers absent today</p>
                </div>

                <div className="admin-summary-strip admin-card">
      ...
      {profileImageOpen && (
        <div
          className="admin-profile-image-modal-overlay"
          onClick={() => setProfileImageOpen(false)}
        >
          <div
            className="admin-profile-image-modal-card"
      ...
      <div className="admin-footer-brand">
        ...
        <img src={abcLogo} alt="Cleezo Class" className="admin-footer-logo" />
      </div>
          ...
          >
            ...
            <div style={{ display: "grid", gap: "0.75rem", marginTop: "0.5rem" }}>
              {selectedCalendarItems.map((item) => (
                <div key={item.id} className="admin-calendar-card-item">
                  <div className="admin-calendar-item-type">{item.type}</div>
                  <div className="admin-calendar-item-name">{item.name}</div>
                </div>
              ))}
            </div>
          </div>
