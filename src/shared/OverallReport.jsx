...
const applyDownloadCleanupStyles = (doc) => {
  ...
  style.textContent = `
    ...
    .report-download-mode .sheet,
    .report-download-mode .report,
    .report-download-mode .container,
    .report-download-mode .page,
    .report-download-mode .card,
    .report-download-mode .report-wrapper,
...
const Header = ({ studentData }) => {
  ...
  if (rawPhoto) {
    ...
    if (typeof photoPath === "string" && (/^https?:\/\//i.test(photoPath) || photoPath.startsWith("data:"))) {
      ...
      photoURL = `https://cleezoclass.com:5000${photoPath}`;
    }
...
const handlePrint = async () => {
    ...
    try {
        const instituteRes = await axios.get(
          `https://cleezoclass.com:5000/api/institute?dbName=${schoolCode}`
        );
...
useEffect(() => {
  const fetchAttendance = async () => {
    ...
    try {
      const response = await axios.post('https://cleezoclass.com:5000/api/report/attendance/monthly', {
        name: studentData.name,
        class_name: studentData.class_name,
        section: studentData.section,
        schoolCode,
