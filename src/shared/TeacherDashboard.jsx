...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const teacherRes = await axios.get('https://cleezoclass.com:5000/teachers', { params: { schoolCode } });
        const currentTeacher = teacherRes.data.find(t => t.name === name);
        setTeacherProfile(currentTeacher);

        const leadsRes = await axios.get("https://cleezoclass.com:5000/api/api/assigned-leads", {
  ...
  const handleDecision = async (leadId, category, decision) => {
    ...
    try {
      const res = await axios.post("https://cleezoclass.com:5000/api/api/teacher-decision", {
