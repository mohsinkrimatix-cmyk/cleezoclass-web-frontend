...
const Extraactivityform = ({
  ...
  const dynamicClasses = classOptions && classOptions.length > 0
    ? classOptions
    : ["Nursery", "LKG", "UKG", ...Array.from({ length: 10 }, (_, i) => String(i + 1))];

  const [selectedOption, setSelectedOption] = useState(predefinedOptions[0]);
  const [customInput, setCustomInput] = useState("");
  const [activityList, setActivityList] = useState([]);
  const [selectedClass, setSelectedClass] = useState(dynamicClasses[0] || "1");

  useEffect(() => {
    if (dynamicClasses.length > 0 && !dynamicClasses.includes(selectedClass)) {
      setSelectedClass(dynamicClasses[0]);
    }
  }, [dynamicClasses, selectedClass]);

  const rawClass = String(selectedClass).replace(/^Class\s+/i, "");
  const classSecs = availableSections[selectedClass] || availableSections[rawClass] || [];
  const currentSections = classSecs.length > 0
    ? (selectedSections.length > 0 ? classSecs.filter((s) => selectedSections.includes(s)) : classSecs)
    : (selectedSections.length > 0 ? selectedSections : ["A", "B", "C"]);

  const finalSections = currentSections.length > 0 ? currentSections : (classSecs.length > 0 ? classSecs : ["A"]);

  const [selectedSection, setSelectedSection] = useState(finalSections[0] || "A");

  useEffect(() => {
    if (finalSections.length > 0 && !finalSections.includes(selectedSection)) {
      setSelectedSection(finalSections[0]);
    }
  }, [finalSections, selectedSection]);
        ...
        <select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}>
          {dynamicClasses.map((cls) => (
            <option key={cls} value={cls}>
              {String(cls).toLowerCase().startsWith("class") || isNaN(Number(cls)) ? cls : `Class ${cls}`}
            </option>
          ))}
        </select>
