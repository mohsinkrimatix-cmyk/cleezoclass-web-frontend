  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    if (flatRows.length && getUploadedRowValue(flatRows[0], ["day"])) {
      flatRows.forEach((row) => {
        ...
        const section =
          getUploadedRowValue(row, ["section", "sec"]) || inferredSection;
        ...
        const fromTime = getUploadedRowValue(row, ["from time", "from_time", "start time", "start"]);
        const toTime = getUploadedRowValue(row, ["to time", "to_time", "end time", "end"]);
    ...
    matrix.slice(headerRowIndex + 1).forEach((row) => {
      ...
      nextTimetable[inferredClassName][inferredSection][day] = periodColumns.map(
        ({ header, index }, columnIndex) => {
          const periodNumber =
            Number(String(header).replace(/\D/g, "")) || columnIndex + 1;
          if (!/^p\d+/i.test(header)) {
...
const normalizeInstituteLogo = (rawLogo) => {
  ...
  if (logo.startsWith("uploads/")) {
    return `https://cleezoclass.com:5000/${logo}`;
  }
  if (logo.startsWith("/uploads/")) {
    return `https://cleezoclass.com:5000${logo}`;
  }
          ...
          {Object.keys(timetable).map((className) => (
            <div key={className} className="at-timetable-class">
              ...
              {Object.keys(timetable[className]).map((section) => {
                ...
                const cleanSection = String(section).replace(/^Section\s+/i, "");
                return (
                  <div key={section} className="at-timetable-section">
                    <h3>Section: {cleanSection}</h3>
...
const downloadTimetableExcel = () => {
  ...
  Object.keys(timetable).forEach((className) => {
    Object.keys(timetable[className]).forEach((section) => {
      ...
      let cleanSection = String(section).replace(/^Section\s+/i, "");
      let sheetName = `Class ${className} - Sec ${cleanSection}`;
  ...
  const downloadTimetablePDF = () => {
    ...
    Object.keys(timetable).forEach((className, classIndex) => {
      Object.keys(timetable[className]).forEach((section, sectionIndex) => {
        ...
        pdf.setFontSize(18);
        pdf.text(
          `Class ${className} - Section ${cleanSection}`,
          14,
          15
        );
