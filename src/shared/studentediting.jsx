...
const managementDesignationOptions = [
    ...
    const fetchMetadata = async () => {
      ...
      try {
        const [classRes, sectionRes] = await Promise.all([
          axios.get(`https://cleezoclass.com:5000/api/admin/classes`, { params: { schoolCode } }),
          axios.get(`https://cleezoclass.com:5000/api/admin/sectionFilter`, { params: { schoolCode } }),
        ]);
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      if (selectedClass && selectedSection) {
        setIsLoading(true);
        try {
          const url = `https://cleezoclass.com:5000/api/users/${selectedClass}`;
          const params = {
  ...
  const uploadPhoto = async () => {
    ...
    try {
      const response = await axios.post(`https://cleezoclass.com:5000/api/upload-photo`, formData, {
        headers: {
  ...
  const handleDelete = async (studentId, userType = 'student') => {
    if (window.confirm(`Are you sure you want to move this ${userType} to deleted list?`)) {
      try {
        await axios.delete(`https://cleezoclass.com:5000/api/users/${studentId}`, {
          params: { schoolCode },
        });
        if (userType === 'teacher') {
          await loadTeacherUsers();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`https://cleezoclass.com:5000/api/users/${editingStudent}`, {
        ...editFormData,
        photo: photoPath
      }, {
        params: { schoolCode }
  ...
  try {
    ...
    if (submittedUserType === 'teacher') {
      ...
      if (teacherPhone) {
        try {
          await axios.post(
            'https://cleezoclass.com:5000/api/campaigning/send-admission-qr-teachers',
            {
              schoolCode,
              teachers: [
                {
    ...
    if (submittedClassName) {
      const updatedStudents = await axios.get(
        `https://cleezoclass.com:5000/api/users/${submittedClassName}`,
        {
          params: {
            schoolCode,
