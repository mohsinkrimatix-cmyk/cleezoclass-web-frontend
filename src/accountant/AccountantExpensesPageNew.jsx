...
const AccountantExpensesPageNew = () => {
  ...
  useEffect(() => {
    const fetchDynamicOptions = async () => {
      try {
        const schoolCode = localStorage.getItem("schoolCode");
        if (!schoolCode) return;
        const response = await fetch(`https://cleezoclass.com:4000/api/admin/get-expense-master?schoolCode=${schoolCode}`);
    ...
    const fetchAssistantLists = async () => {
      try {
        ...
        const [nextExpenseRows, salaryResponse] = await Promise.all([
          ...
          axios.get("https://cleezoclass.com:4000/api/salarymanagement", { params: { schoolCode } }),
        ]);
  ...
  const confirmDeleteExpense = async () => {
    ...
    try {
      const schoolCode = localStorage.getItem("schoolCode");
      await axios.delete(`https://cleezoclass.com:4000/Accountntdata/${expenseToDelete.id}?schoolCode=${schoolCode}`);
...
const handleSaveCategory = async () => {
  ...
  try {
    await axios.post(
      "https://cleezoclass.com:4000/api/admin/add-expense",
      {
        schoolCode,
  ...
  const handleSubmitExpense = async () => {
    ...
    try {
      ...
      if (editingExpenseId) {
        ...
        await axios.put("https://cleezoclass.com:4000/Accountntdata", payload);
