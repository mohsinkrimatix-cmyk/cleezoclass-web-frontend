...
const normalizePhotoUrl = (rawPhoto) => {
  ...
  return `https://cleezoclass.com:5000${photoPath}`;
};
...
const handlePageChange = (newPage) => {
  ...
  useEffect(() => {
    const loadSchoolDetails = async () => {
      ...
      try {
        ...
        const url = `https://cleezoclass.com:5000/api/school-lookup?${query.toString()}`;
  ...
  useEffect(() => {
    const loadRoleUsers = async () => {
      ...
      try {
        setRoleLoading(true);
        const response = await axios.post('https://cleezoclass.com:5000/api/users', { schoolCode, user_type: roleTabConfig[activeRoleTab].userType });
  ...
  const uploadStudentPhoto = async () => {
    ...
    const response = await axios.post('https://cleezoclass.com:5000/api/upload-photo', uploadData, { headers: { 'Content-Type': 'multipart/form-data' }, params: { schoolCode } });
  ...
  const handleAddUserSubmit = async (e) => {
    ...
    try {
      ...
      if (userModalMode === 'edit' && editingUserId != null) {
        ...
        await axios.put(`https://cleezoclass.com:5000/api/users/${editingUserId}`, editPayload, { params: { schoolCode }, headers: { 'Content-Type': 'application/json' } });
        ...
        const response = await axios.post('https://cleezoclass.com:5000/api/submit-studentData', addPayload, { params: { schoolCode }, headers: { 'Content-Type': 'application/json' } });
  ...
  const handleUpdate = () => {
    ...
    axios.put(`https://cleezoclass.com:5000/api/api/schooleditingdatacentralization/${schoolCode}`, uploadData, { headers: { 'Content-Type': 'multipart/form-data' } }).then(() => alert('Updated successfully')).catch(() => alert('Update failed'));
  };
  ...
  const handleFileUpload = async (category) => {
    ...
    try {
      ...
      const response = await axios.post(`https://cleezoclass.com:5000/api/upload-excel/${apiCategory}`, uploadData, { params: { schoolCode }, headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const deleteUser = async (user) => {
    ...
    try { setRoleLoading(true); setActionUser(null); await axios.delete(`https://cleezoclass.com:5000/api/users/${userId}`, { params: { schoolCode }, data: { schoolCode } }); await reloadRoleUsers(); alert('Deleted permanently'); } 
  ...
  const toggleUserDisabled = async (user) => {
    ...
    try { setRoleLoading(true); setActionUser(null); await axios.patch(`https://cleezoclass.com:5000/api/users/${userId}/disabled`, { schoolCode, disabled }, { params: { schoolCode } }); await reloadRoleUsers(); alert(disabled ? 'User disabled.' : 'User enabled.'); } 
  ...
  useEffect(() => {
    const schoolCode = formData.school_code || localStorage.getItem('schoolCode');
    if (!schoolCode) return;
    axios.get(`https://cleezoclass.com:5000/track-records/${schoolCode}`).then((res) => { if (res.data.success) setTrackingStats(res.data.stats); }).catch((err) => console.error(err.message));
