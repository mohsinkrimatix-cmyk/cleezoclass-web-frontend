...
const CreateMeeting = ({ onCreated }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post('https://cleezoclass.com:5000/create-meeting', {
      ...
      {meetingId && (
        ...
        >
          ...
          <p>
            ...
            <a
              href={`https://cleezoclass.com:5000/meeting/${meetingId}/video`}
              target="_blank"
              rel="noopener noreferrer"
              style={{ color: '#007bff', textDecoration: 'none' }}
            >
              {`https://cleezoclass.com:5000/meeting/${meetingId}/video`}
            </a>
          </p>
        </div>
      )}
...
const SlotPicker = ({ onSlotConfirmed, roomId }) => {
  ...
  useEffect(() => {
    const fetchAvailableSlots = async () => {
      try {
        const res = await axios.get(`https://cleezoclass.com:5000/available-slots?meetingId=${roomId}`);
  ...
  const handleConfirmSlot = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post('https://cleezoclass.com:5000/select-slot', {
        meetingId: roomId,
        slotTime,
      });
      setSubmitted(true);
...
useEffect(() => {
  ...
  const checkMeetingStatus = async () => {
    ...
    try {
      const res = await axios.get(`https://cleezoclass.com:5000/validate-meeting/${effectiveRoomId}`);
