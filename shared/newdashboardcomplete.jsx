...
useEffect(() => {
  axios.get('https://cleezoclass.com:5000/api/approved')
...
// Function to generate an array of dates between two Date objects (omitted for brevity, assume unchanged)
 const [showPending, setShowPending] = useState(false);
  ...
  const fetchPending = () => {
    ...
    axios
      .get("https://cleezoclass.com:5000/api/pending")
      .then((res) => {
...
const fetchFeeData = async () => {
  ...
  try {
    ...
    const res = await axios.post(
      `https://cleezoclass.com:5000/api/feeDataFinanceNew?schoolCode=${schoolCode}`
    );
...
const handleSearch = async () => {
  ...
  try {
    ...
    const expensesPromise = axios.get(
      `https://cleezoclass.com:5000/api/totalexpensesData?schoolCode=${schoolCode}`
    );
