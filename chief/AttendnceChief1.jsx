...
const fetchReport = async (monthYear) => {
  ...
  try {
    const res = await axios.get(
      `https://cleezoclass.com:5000/api/api/salary-report`,
...
const fetchReport1 = async (monthYear) => {
  ...
  try {
    ...
    const deductionRes = await axios.get(
      `https://cleezoclass.com:5000/api/api/monthly-deductions`,
      { params: { month: monthYearFormatted, schoolCode } }
    );

    // 2. Latecomers
    const lateRes = await axios.get(
      `https://cleezoclass.com:5000/api/teacher-list-of-latecomers`,
      { params: { schoolCode, month, year } }
    );

    // 3. Absentees
    const absentRes = await axios.get(
      `https://cleezoclass.com:5000/teacher-list-of-irregulars-time`,
      { params: { schoolCode, month, year } }
    );
